...
@class Pointzi;
@compatibility_alias SHApp Pointzi;
 ...
 */
#define StreetHawk          [Pointzi sharedInstance]
#define POINTZI             [Pointzi sharedInstance]
 ...
 */
@interface Pointzi : NSObject<UIApplicationDelegate>
 ...
 */
+ (nonnull Pointzi *)sharedInstance;
...
/**
 ...
 @param isDebugMode The mode of whether print NSLog in Xcode console.
 */
- (void)registerInstallForApp:(nonnull NSString *)appKey withDebugMode:(BOOL)isDebugMode completion:(void (^_Nullable)(void))completion;
...
/**
 ...
 3. If 1 and 2 still nill or empty, check InfoPlist `APP_KEY`.
 @param isDebugMode The mode of whether print NSLog in Xcode console.
 @param isSuperTagsEnabled The mode enable's sending supertags.
 */
- (void)registerInstallForApp:(nonnull NSString *)appKey withDebugMode:(BOOL)isDebugMode withSuperTags:(BOOL)isSuperTagsEnabled completion:(void (^_Nullable)(void))completion;
 ...
 */
- (void)registerInstallForApp:(nonnull NSString *)appKey segmentId:(NSString *)segmentId withDebugMode:(BOOL)isDebugMode completion:(void (^_Nullable)(void))completion;
...
/**
 Decide whether need to show debug log in console.
 */
@property (nonatomic) BOOL isSuperTagsEnabled;
...
/**
  Tracks the Application life cycle state
 */
@property (nonatomic) UIApplicationState applicationLifeCycleState;

/**
 read local feeds for testing purpose
 */
@property (nonatomic) NSString * _Nullable loadLocalFeeds;

/**
 Tracks modified preview feeds
 */
@property NSDate * _Nullable feedsModifiedTime;

/**
 Tracks modified live feeds
 */
@property NSDate * _Nullable liveFeedsModifiedTime;

/**
 Tracks modified preview feeds
 */
@property (nonatomic) BOOL pollingInitialFeeds;

/**
 Fetch feeds starting from `offset`.
 */
@property (nonatomic) NSUInteger pagingFeedOffset;

/**
 Paging feed iDS
 */
//@property (nonatomic) NSString * _Nullable pagingFeedIDs;

@property (nonatomic, strong) NSTimer * _Nullable timerPoll;
@property (assign) NSTimeInterval timeInterval;

@property (nonatomic, strong, nullable) NSString *onDemandCampaignID;

@property (nonatomic, strong, nullable) NSString *onDemandVC;

@property (nonatomic,strong) NSMutableArray * _Nullable pollingFeedArray;
/**
...
- (void)shSendFeedbackWithTitle:(nullable NSString *)title
                 ...
                 withCampaignID:(nullable NSString *)campaignID
                       withStep:(NSInteger )stepId
                      withFeedId:(nullable NSString *)feed_id
...
- (void)registerInstallForApp:(nonnull NSString *)appKey withDebugMode:(BOOL)isDebugMode withLocalFeed:(NSString *_Nullable)withLocalFeed;
...
- (BOOL)tagCuid:(id _Nullable )uniqueId;
...
/**
 Tag user country based on locale
 */
- (BOOL)tagUserCountry;
...
/**
 setReactViewName
 @param viewName react native js component view name
 */
- (void)setReactViewName:(NSString *)viewName;
...
/**
 For some reason server loses all the install, sdk re - register local install on the server .
 @param install Current install.
 */
- (void)reRegisterOnServerInstallFailure:(nullable SHInstall *)install andHandler:(SHCallbackHandler)handler;
...
/**
Shows the onDemand campaign
@param campaignId  campaignID
*/
+(void)showOnDemandCampaign:(nonnull NSString *)campaignId NS_SWIFT_NAME(showOnDemandCampaign(_:));

/**
 Get pointzi version.
 */
+ (NSString *_Nonnull)pointziVersion;

/**
Verify the installed version with latest released version
*/
- (void)verifySDKVersion;
