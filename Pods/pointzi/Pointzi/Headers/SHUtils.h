...
extern NSString *shNextRepeatConditonDate(NSString * timeUnit, NSString *type);

extern NSString *shParseRepeatConditionTimeUnits(NSString * timeUnit, NSString *type);
...
/**
 Deserialize the json string to json Dictionary.
 @param obj The string to be deserialized.
 @return The dictionary  if deserialize successfully. If fail return nil.
 */
extern NSDictionary *shDeSerializeJsonStringToDict(NSString *str);

/**
Serialize the Dictionary to  json string.
@param dict to be serialized.
@return The Json string  if serialization successfully. If fail return nil.
*/
extern NSString *shSerializeDicToJsonString(NSDictionary *dict);
 ...
 */
extern NSString *shConvertSHTipTypeString(int type);
...
extern UIWindow *shKeyWindow(void);
...
/**
@param key  The key in the userdefaults
@param value The value to be save
 */
extern void shSaveToUserDefaults(NSString *key, int value);


/**
Determine if the featured is enabled based on app status
@param key  The key in the userdefaults
*/
extern BOOL isFeatureEnabled(NSString *key ,NSObject *lastSaveTime);
...
typedef enum SHViewTag SHViewTag;
...
/**
Return app/status result of supertags  should be enabled.
*/
extern BOOL streetHawkSuperTagEnabled(void);


/**
Return app/status result of supertags  should be enabled.
*/
extern BOOL isSuperTagEnabled(SHViewTag tag);


/**
Return app/status result of supertags  filter is passed
*/
extern BOOL isSuperTagFilterPassed(NSString* pattern);
...
/**
 Check whether SDK's Integration is Correct.
 */
extern BOOL IsSDKIntegrationCorrect(UIViewController * vc);
 ...
 */
+ (UIColor *)pzColorFromString:(NSString *)str;

/**
 Convert color to hex string formatted as #AARRGGBB. If color is nil return nil.
 */
+ (NSString *)pzHexStringFromColor:(UIColor *)color;
